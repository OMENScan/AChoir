...
VOID DumpDataII(ULONG index, CHAR* filename, FILETIME ToCreTime, FILETIME ToModTime, FILETIME ToAccTime, int binLog);
  ...
  if (dbMrc == SQLITE_OK)
  {
    ...
    while ((dbMrc = sqlite3_step(dbMFTStmt)) != SQLITE_DONE)
    {
      ...
      if (dbMrc == SQLITE_ROW)
      {
        ...
        printf("Inf: Raw Copying FileName: %s\nMFT Record: %d\n", Full_Fname + i + 1, Full_MFTID);
        printf("    %s\n", Full_Fname);
        fprintf(LogHndl, "Inf: Raw Copying FileName: %s\nMFT Record: %d\n", Full_Fname + i + 1, Full_MFTID);
        fprintf(LogHndl, "    %s\n", Full_Fname);
        ...
        printf("Meta: %ld - %ld - %ld\n", File_CreDate, File_AccDate, File_ModDate);
        printf("SID: %s\n", SidString);
        fprintf(LogHndl, "Meta: %ld - %ld - %ld\n", File_CreDate, File_AccDate, File_ModDate);
        fprintf(LogHndl, "SID: %s\n", SidString);

        DumpDataII(Full_MFTID, Full_Fname + i + 1, File_Create, File_Modify, File_Access, 1);
      ...
      if (file->Flags == 1)
      {
        ...
        else
        {
          if (attr3 == 0)
            ...
            dbMQuery = sqlite3_mprintf("INSERT INTO MFTFiles (MFTRecID, MFTPrvID, FileName, FileCreDate, FileAccDate, FileModDate, FileDateTyp, FNCreDate, FNAccDate, FNModDate, SICreDate, SIAccDate, SIModDate) VALUES ('%ld', '%ld', '%q', 'SI', '%llu', '%llu', '%llu', '%llu', '%llu', '%llu')\0",
              i, int(name2->DirectoryFileReferenceNumber), Str_Temp,
              ULONGLONG(name3->CreationTime), ULONGLONG(name3->LastAccessTime), ULONGLONG(name3->LastWriteTime),
              ULONGLONG(name2->CreationTime), ULONGLONG(name2->LastAccessTime), ULONGLONG(name2->LastWriteTime),
              ULONGLONG(name3->CreationTime), ULONGLONG(name3->LastAccessTime), ULONGLONG(name3->LastWriteTime));
...
VOID DumpDataII(ULONG index, CHAR* filename, FILETIME ToCreTime, FILETIME ToModTime, FILETIME ToAccTime, int binLog)
{
  ...
  snprintf(Tooo_Fname, 2040, "%s\\%s\0", ACQDir, filename);
  ...
  if (access(Tooo_Fname, 0) == 0)
  {
    do
    {
      ...
      memset(Tooo_Fname, 0, 2048);
      snprintf(Tooo_Fname, 2040, "%s\\%s(%d)\0", ACQDir, filename, iFileCount);
