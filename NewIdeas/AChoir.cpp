...
// NTP Variables
char ntpFQDN[255] = "\0";
char ntpDateTime[50] = "Sat Nov 11 17:11:17 2017\0";
...
/************************************************************/
/* Get NTP Time                                             */
/* This code is  Mish-Mosh of coe I grabbed from all over   */
/*  the Internet - Including * (C) 2014 David Lettier's     */
/*  code at www.lettier.com                                 */
/*                                                          */
...
int ntpGetTime(char* ntpServer)
{
  ...
  {
    consPrefix("\n[!] ", consRed);
    fprintf(LogHndl, "[!] Winsock could not startup.\n");
    printf("Winsock could not startup.\n");
    WSACleanup();
    return(1);
  }
  ...
  {
    consPrefix("\n[!] ", consRed);
    fprintf(LogHndl, "[!] Winsock 1.1 is not supported.\n");
    printf("Winsock 1.1 is not supported.\n");
    WSACleanup();
    return(2);
  }
  ...
  {
    consPrefix("\n[!] ", consRed);
    fprintf(LogHndl, "[!] ERROR opening socket.\n");
    printf( "ERROR opening socket.\n" );
    return(3);
  }

  ntpXServer = gethostbyname(ntpServer); // Convert URL to IP.

  if ( ntpXServer == NULL )
  {
    consPrefix("\n[!] ", consRed);
    fprintf(LogHndl, "[!] ERROR, no such host..\n");
  ...
  *(( char *) &ntpOut + 0) = 0x1b;
  ...
  {
    consPrefix("\n[!] ", consRed);
    fprintf(LogHndl, "[!] Could not Set Socket TimeOut.\n");
  ...
  if (ntpRecvRC > 0)
  {
    ...
    sprintf(ntpDateTime, "%s", ctime((const time_t*) &txTm ));
  }
  else
  if (ntpRecvRC == 0 )
  {
