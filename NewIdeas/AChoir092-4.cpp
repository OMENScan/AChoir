...
int main(int argc, char *argv[])
{
  ...
  /****************************************************************/
  iSigCount = 0;
  ClustZero = new UCHAR[bootb.BytesPerSector * 2]; // Files Sector 0 Buffer
  if(ClustZero == NULL) 
    ...
    while ((dbMrc = sqlite3_step(dbMFTStmt)) != SQLITE_DONE)
    {
      ...
      if (dbMrc == SQLITE_ROW)
      {
        ...
        if (DDRetcd == 0)
          ...
          if (SecDesc !=NULL)
          {
printf("Free SecDesc: Start");
            free(SecDesc);
printf("Free SecDesc: End");
          }
