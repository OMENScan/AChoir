...
VOID DumpDataII(ULONG index, CHAR* filename, FILETIME ToCreTime, FILETIME ToModTime, FILETIME ToAccTime, int binLog)
{
  ...
  snprintf(Tooo_Fname, 2040, "%s\\%s\\%s\0", BACQDir, ACQDir, filename);
  ...
  {
    fprintf(LogHndl, "Inf: Destination File MD5: %s\n", MD5Out);
    fprintf(LogHndl, "Inf: Destination MetaData: %ld-%llu-%llu-%llu\n", Toostat.st_size, ftCreate, ftAccess, ftWrite);
  }
  printf("Inf: Destination File MD5: %s\n", MD5Out);
  printf("Inf: Destination MetaData: %ld-%llu-%llu-%llu\n", Toostat.st_size, ftCreate, ftAccess, ftWrite);
}
